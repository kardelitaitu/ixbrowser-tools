...
exports.simulateMouseMovement = simulateMouseMovement;
exports.randomDelay = randomDelay;
exports.addHumanClickToPage = addHumanClickToPage;
exports.addHumanScrollToPage = addHumanScrollToPage;
 ...
 */
async function simulateMouseMovement(page, element, options = {}) {
    const { steps = Math.floor(Math.random() * 15) + 10, // More steps for smoother curves
    minDelay = 15, maxDelay = 60, overshoot = Math.random() < 0.3, // 30% chance to overshoot slightly
     } = options;
    try {
        ...
        const controlPoint1X = startX + (targetX - startX) * (0.2 + Math.random() * 0.3) + (Math.random() - 0.5) * 100;
        const controlPoint1Y = startY + (targetY - startY) * (0.2 + Math.random() * 0.3) + (Math.random() - 0.5) * 100;
        const controlPoint2X = startX + (targetX - startX) * (0.5 + Math.random() * 0.3) + (Math.random() - 0.5) * 100;
        const controlPoint2Y = startY + (targetY - startY) * (0.5 + Math.random() * 0.3) + (Math.random() - 0.5) * 100;
 ...
 */
function addHumanClickToPage(page) {
    page.humanClick = async function (selector) {
        try {
 ...
 * @param page - The Playwright page instance.
 */
function addHumanScrollToPage(page) {
    page.humanScroll = async function (direction, amount) {
        try {
            ...
            while (scrolled < totalScroll) {
                const scrollAmount = Math.min(Math.floor(Math.random() * 100) + 50, totalScroll - scrolled); // Scroll 50-150px at a time
            ...
            console.log('Human scroll failed:', error.message);
        }
    };
}
/**
 ...
 */
async function humanScroll(page, direction = 'down', amount = 300, options = {}) {
    const { delay } = options;
    try {
        const scrollDistance = direction === 'down' ? amount : -amount;
        // Simulate multiple smaller scroll events with variable delays
